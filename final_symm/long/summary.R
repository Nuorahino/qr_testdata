...
tikz('../../../thesis/tikz/compare_long.tex',width=7,height=4.5)
#ggp <-ggplot(filter(res_table, hermitian == 2, complex == 0, tol == 1e-16, size >= 900, version != "Eigen"), aes(size, avg_runtime, colour = version)) + geom_smooth(aes(group=version)) #+ geom_point()
...
tikz('../../../thesis/tikz/compare_long.tex',width=7,height=4.5)
ggp <- ggplot(filter(res_table, hermitian == 1, complex == 0, tol == 1e-16, size >= 900), aes(size, avg_error, colour = version)) + geom_smooth(aes(group=version)) #+ facet_wrap(~bin, scale='free_y', ncol = 1)
ggp + labs(x =  "size", y = "average error") + theme(strip.text.x = element_blank())
dev.off()
